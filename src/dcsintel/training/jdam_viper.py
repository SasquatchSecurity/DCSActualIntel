"""F-16 JDAM strike curriculum: TGP designate, GPS weapon employment."""
...
def build_jdam_viper(spec: dict, out_path: str | None = None) -> str:
    prof = spec["difficulty_profile"]
    messages = load_data("curricula")["curricula"]["jdam_viper"]["messages"]
    ...
    for tgt in targets:
        fg.add_waypoint(tgt, int(14000 * FT))
    ...
    zone_brief(
        m,
        targets[0].point_from_heading((heading + 180) % 360, 10 * NM),
        approach_radius,
        player.id,
        2,
        messages["phase2_designate"],
        60,
    )
    ...
    if target_count > 1:
        zone_brief(
            m,
            targets[1].point_from_heading((heading + 180) % 360, 12 * NM),
            approach_radius,
            player.id,
            3,
