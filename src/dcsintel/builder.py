...
def _apply_briefing(ctx: BuildContext) -> None:
    ...
    if prof := spec.get("difficulty_profile"):
        lines.append(f"Threat level: {prof['label']}")
    if spec.get("twists"):
        from .twists import TWIST_DEFS

        labels = [TWIST_DEFS[t]["label"] for t in spec["twists"] if t in TWIST_DEFS]
        if labels:
            lines.append(f"Scenario factors: {', '.join(labels)}")
    lines.extend(spec.get("_twist_briefing_lines", []))
    lines.extend([
        "",
        *ctx.briefing_lines,
        "",
